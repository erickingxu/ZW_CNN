...
vector<Mat> Convolution::Activation() {
	...
	if (kernel_stride_height == 1 && kernel_stride_width == 1) {
		...
		for (int i = 0; i < output_length; i += input_length) {
			if (input_length == 1) {
				IM2COL ZW(input[i], kernel[i], kernel_stride_height, kernel_stride_width, padding);
				output[i] = ZW.ZW_IM2COL();
			}
			else {
				vector <Mat> temp;
				for (int j = 0; j < input_length; j++) {
					IM2COL ZW(input[j], kernel[i + j], kernel_stride_height, kernel_stride_width, padding);
